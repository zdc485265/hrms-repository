												...
												<div class="widget-header">
													...
													<div class="widget-toolbar no-border">
														<ul class="nav nav-tabs" id="myTab2">
															<li >
																<a data-toggle="tab" href="javascript:void(0)" onclick="window.location.href='${pageContext.request.contextPath}/admin/toAddAdmin.gkd'">新增</a>
															</li>
												...
												<div class="widget-body">
													<div class="widget-main padding-12 no-padding-left no-padding-right">
														<div class="tab-content padding-4">
													        ...
													        <p style="color: red;">${requestScope.admin_list_msg }</p>
															<div id="adminList" class="tab-pane in active">
															<button onclick="deleteAdminById()"  class="btn btn-primary">删除勾选用户</button>
																 ...
																 <table class="table table-striped table-bordered align-center "> 
																   ...
																   <c:forEach var="admin" varStatus="status" items="${requestScope.page.data }">
																	   <tr>
																		  <td><input name="admin_id"  type="checkbox" value="${admin.admin_id }"></td>
																		  <td>${admin.admin_id }</td>
																		  <td>${admin.admin_name }</td>
																		   <td>${admin.admin_account }</td>
																		   <c:choose>
																		      <c:when test="${admin.admin_status==0 }">
																		          <td>可用</td>
																		      </c:when>
																		      <c:otherwise>
																		          <td>禁用</td>
																		  ...
																		  <td>${admin.role.role_name }</td>
																		   <td><button class="btn btn-sm btn-primary" onclick="window.location.href='${pageContext.request.contextPath}/admin/toEditAdmin.gkd?admin_id=${admin.admin_id }'">编辑</button>&nbsp;&nbsp;<button onclick="window.location.href='${pageContext.request.contextPath}/admin/deleteAdmin.gkd?admin_id=${admin.admin_id}'" class="btn btn-sm btn-primary">删除</button></td>
																		</tr>
																	</c:forEach>
																 ...
																 <div class="text-right">
																	 <nav>
																	  <ul class="pagination">
																	 		<li>
																		      			<a href="${pageContext.request.contextPath }/admin/toAdminList.gkd?index=0">首页</a>
																		     </li>
																	      <c:choose>
																	         <c:when test="${requestScope.page.previous }">
																	                 <li>
																	               <a href="${pageContext.request.contextPath }/admin/toAdminList.gkd?index=${requestScope.page.index-1 }" aria-label="Previous">
																		...
																		<c:forEach begin="1" varStatus="status" end="${requestScope.page.totalPage }">
																		       ...
																		       <c:choose>
																		          ...
																		          <c:when test="${status.count==requestScope.page.index+1}">
								                                                         <%-- 注意事项：数据库的索引是从0开始的，而页面索引是从1开始的。索引数据索引=页面索引-1 --%>
																		      			<li class="active">
																		      			<a href="${pageContext.request.contextPath }/admin/toAdminList.gkd?index=${status.count-1}">${status.count }</a>
																		      			</li>										          
																		          </c:when>
																		          <c:otherwise>
																		                <li>
																	     ...
																	     <c:choose>
																	        <c:when test="${requestScope.page.next }">
																	           <li>
																	             <a href="${pageContext.request.contextPath }/admin/toAdminList.gkd?index=${requestScope.page.index+1}" aria-label="Next">
																	      ...
																	      <li>
																		      			<a href="${pageContext.request.contextPath }/admin/toAdminList.gkd?index=${requestScope.page.totalPage-1}">尾页</a>
																		     </li>
...
<![endif]-->
		...
		<script type="text/javascript">
		   ...
		   //声明一个方法
		   var deleteAdminById=function(){
			   //alert("--deleteAdminById-");
			   //1.获得选中 的复选框,使用serialize()方法可以将参数以格式：chkAdminId=3&chkAdminId=4 发送
			   var arr=$("input[name='admin_id']:checked").serialize();
			   console.log(arr);
			   //2.获得对象的值，发送到后台
			   window.location.href="${pageContext.request.contextPath}/admin/deleteAdmin.gkd?"+arr;
		   }
