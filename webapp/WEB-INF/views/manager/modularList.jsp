					...
					<div class="page-content">
				          			<div class="widget-box transparent" id="widget-box-13">
												<div class="widget-header">
													<h4 class="widget-title lighter">模块列表</h4>

													<div class="widget-toolbar no-border">
														<ul class="nav nav-tabs" id="myTab2">
															<li >
																<a data-toggle="tab" href="javascript:void(0)" onclick="window.location.href='${pageContext.request.contextPath}/modular/toAddModular.gkd'">新增</a>
															</li>
												...
												<div class="widget-body">
													<div class="widget-main padding-12 no-padding-left no-padding-right">
														<div class="tab-content padding-4">
													        ...
													        <p style="color: red;">${requestScope.modular_list_msg }</p>
															<div id="modularList" class="tab-pane in active">
															<button  onclick="deleteModularById()" class="btn btn-primary">删除勾选模块</button>
																 ...
																 <table class="table table-striped table-bordered align-center "> 
																    ...
																    <c:forEach var="modular" varStatus="status" items="${requestScope.page.data }">
																	   <tr>
																		  <td><input name="modular_id"  type="checkbox" value="${modular.modular_id }"></td>
																		  <td>${modular.modular_id }</td>
																		  <td>${modular.modular_name }</td>
																		 <td>${modular.modular_sort}</td>
																		   <td><button class="btn btn-sm btn-primary" onclick="window.location.href='${pageContext.request.contextPath}/modular/toEditModular.gkd?modular_id=${modular.modular_id }'">编辑</button>&nbsp;&nbsp;<button onclick="window.location.href='${pageContext.request.contextPath}/modular/deleteModular.gkd?modular_id=${modular.modular_id}'" class="btn btn-sm btn-primary">删除</button></td>
																		</tr>
																	</c:forEach>
																 ...
																 <div class="text-right">
																	 <nav>
																	  <ul class="pagination">
																	 <li>
																		      			<a href="${pageContext.request.contextPath }/modular/toModularList.gkd?index=0">首页</a>
																		      			</li>
																	      <c:choose>
																	         <c:when test="${requestScope.page.previous }">
																	                 <li>
																	               <a href="${pageContext.request.contextPath }/modular/toModularList.gkd?index=${requestScope.page.index-1 }" aria-label="Previous">
																		...
																		<c:forEach begin="1" varStatus="status" end="${requestScope.page.totalPage }">
																		       ...
																		       <c:choose>
																		          ...
																		          <c:when test="${status.count==requestScope.page.index+1}">
								                                                         <%-- 注意事项：数据库的索引是从0开始的，而页面索引是从1开始的。索引数据索引=页面索引-1 --%>
																		      			<li class="active">
																		      			<a href="${pageContext.request.contextPath }/modular/toModularList.gkd?index=${status.count-1}">${status.count }</a>
																		      			</li>										          
																		          </c:when>
																		          <c:otherwise>
																		                <li>
																	     ...
																	     <c:choose>
																	        <c:when test="${requestScope.page.next }">
																	           <li>
																	             <a href="${pageContext.request.contextPath }/modular/toModularList.gkd?index=${requestScope.page.index+1}" aria-label="Next">
																	      ...
																	      <li>
																		      			<a href="${pageContext.request.contextPath }/modular/toModularList.gkd?index=${requestScope.page.totalPage-1}">尾页</a>
																		      			</li>
		...
		<!-- inline scripts related to this page -->
		   ...
		   $("#chkAllModular").click(function(){
			   ...
			   if($(this).prop("checked")){
				   ...
				   $("input[name='modular_id']").prop("checked",true);
			   }else{
				   $("input[name='modular_id']").prop("checked",false);
			   }
		   });
		   
		   //声明一个方法
		   var deleteModularById=function(){
			   //alert("--deleteModularById-");
			   //1.获得选中 的复选框,使用serialize()方法可以将参数以格式：chkModularId=3&chkModularId=4 发送
			   var arr=$("input[name='modular_id']:checked").serialize();
			   console.log(arr);
			   //2.获得对象的值，发送到后台
			   window.location.href="${pageContext.request.contextPath}/modular/deleteModular.gkd?"+arr;
		   }
