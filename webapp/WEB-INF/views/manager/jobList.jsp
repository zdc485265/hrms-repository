		...
		<div class="main-content-inner">
			...
			<div class="page-content">
				...
				<div class="widget-header">
					<h4 class="widget-title lighter">职位列表</h4>

					<div class="widget-toolbar no-border">
						<ul class="nav nav-tabs" id="myTab2">
							<li >
								<a data-toggle="tab" href="javascript:void(0)" onclick="window.location.href='${pageContext.request.contextPath}/job/toAddJob.gkd'">新增</a>
							</li>

					

							<li class="active">
								<a data-toggle="tab" href="#jobList">列表</a>
							</li>
						</ul>
					</div>
				</div>
				...
				<div class="widget-body">
					<div class="widget-main padding-12 no-padding-left no-padding-right">
						<div class="tab-content padding-4">
					        ...
					        <p style="color: red;">${requestScope.job_list_msg }</p>
							<div id="jobList" class="tab-pane in active">
							<button onclick="deleteJobById()" class="btn btn-primary">删除勾选职位</button>
									
								 <table class="table table-striped table-bordered align-center "> 
								    <tr class="bolder" > 
									  <td><input id="chkAlljob" type="checkbox"></td>
									  <td>职位编码</td>
									  <td>职位名称</td>
									  <td>职位分类</td>
									  <td>所属部门</td>
								   ...
								   <c:forEach var="job" varStatus="status" items="${requestScope.page.data }">
									   <tr>
										  <td><input name="job_id"  type="checkbox" value="${job.job_id }"></td>
										  <td>${job.job_code }</td>
										  <td>${job.job_name }</td>
										   <td>${job.classify.classify_name}</td>
										    <td>${job.dept.dept_name }</td>
										   <c:choose>
										      <c:when test="${job.job_status==0 }">
										          <td>可用</td>
										      </c:when>
										      <c:otherwise>
										          <td>禁用</td>
										   ...
										   <td><button class="btn btn-sm btn-primary" onclick="window.location.href='${pageContext.request.contextPath}/job/toEditJob.gkd?job_id=${job.job_id }'">编辑</button>
										   &nbsp;&nbsp;<button onclick="window.location.href='${pageContext.request.contextPath}/job/deleteJob.gkd?job_id=${job.job_id }'" class="btn btn-sm btn-primary">删除</button>
										   &nbsp;&nbsp;<button onclick="window.location.href='${pageContext.request.contextPath}/job/toJobDetailed.gkd?job_id=${job.job_id }'" class="btn btn-sm btn-primary">明细</button>
										   </td>
										</tr>
									</c:forEach>
								 ...
								 <div class="text-right">
									 <nav>
									  <ul class="pagination">
									  <li>
									                <a href="${pageContext.request.contextPath }/job/toJobList.gkd?index=0">首页</a>
									                </li>
									      <c:choose>
									         <c:when test="${requestScope.page.previous }">
									                 <li>
									               <a href="${pageContext.request.contextPath }/job/toJobList.gkd?index=${requestScope.page.index-1 }" aria-label="Previous">
									...
									<c:forEach begin="1" varStatus="status" end="${requestScope.page.totalPage }">
									     ...
									     <c:if test="${status.count>=s&&status.count<=e }">
									       <c:choose>
									          ...
									          <c:when test="${status.count==requestScope.page.index+1}">
									      			...
									      			<li class="active">
									      			<a href="${pageContext.request.contextPath }/job/toJobList.gkd?index=${status.count-1}">${status.count }</a>
									      			</li>										          
									          </c:when>
									          <c:otherwise>
									                <li>
									     ...
									     <c:choose>
									        <c:when test="${requestScope.page.next }">
									           <li>
									             <a href="${pageContext.request.contextPath }/job/toJobList.gkd?index=${requestScope.page.index+1}" aria-label="Next">
									    ...
									    <li>
									                <a href="${pageContext.request.contextPath }/job/toJobList.gkd?index=${requestScope.page.totalPage-1}">尾页</a>
									                </li>
...
<!-- inline scripts related to this page -->
		<script type="text/javascript">
		   ...
		   //声明一个方法
		   var deleteJobById=function(){
			   //alert("--deleteRoleById-");
			   //1.获得选中 的复选框,使用serialize()方法可以将参数以格式：chkRoleId=3&chkRoleId=4 发送
			   var arr=$("input[name='job_id']:checked").serialize();
			   console.log(arr);
			   //2.获得对象的值，发送到后台
			   window.location.href="${pageContext.request.contextPath}/job/deleteJob.gkd?"+arr;
		   }
			</script>
	</body>
</html>
